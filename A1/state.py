...
class State:
    ...
    def __init__(
        ...
        if force_valid:
            ...
            assert isinstance(
                r, int
            ), "!!! tried to create State but BEE_posit row is not an integer !!!"
            assert isinstance(
                c, int
            ), "!!! tried to create State but BEE_posit col is not an integer !!!"
            assert (
                0 <= r < environment.n_rows
            ), "!!! tried to create State but BEE_posit row is out of range !!!"
            assert (
                0 <= c < environment.n_cols
            ), "!!! tried to create State but BEE_posit col is out of range !!!"
            assert (
                BEE_orient in BEE_ORIENTATIONS
            ), "!!! tried to create State but BEE_orient is not a valid orientation !!!"
            assert (
                len(widget_centres) == environment.n_widgets
            ), "!!! tried to create State but number of widget positions does not match environment !!!"
            assert (
                len(widget_orients) == environment.n_widgets
            ), "!!! tried to create State but number of widget orientations does not match environment !!!"
            for i in range(environment.n_widgets):
                assert (
                    widget_orients[i] in WIDGET_ORIENTS[environment.widget_types[i]]
    ...
    def __eq__(self, other):
        if not isinstance(other, State):
            return False
        return (
            self.BEE_posit == other.BEE_posit
            and self.BEE_orient == other.BEE_orient
            and self.widget_centres == other.widget_centres
            and self.widget_orients == other.widget_orients
        )

    def __hash__(self):
        return hash(
            (self.BEE_posit, self.BEE_orient, self.widget_centres, self.widget_orients)
        )
